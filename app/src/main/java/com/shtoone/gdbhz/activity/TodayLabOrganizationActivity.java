...
public class TodayLabOrganizationActivity extends BaseActivity {
    private static final String TAG = TodayLabOrganizationActivity.class.getSimpleName();
...
//        treeNodes.clear();
    ...
    public void responseOrganization(List<OrganizationNode> list) {
        ...
        try {
            mAdapter = new OrganizationTreeListViewAdapter(treeListView, this, list, 2);
            ...
            public void onClick(Node node, int position) {
                ...
                if ("1".equals(mParametersData.userType)) {
                    if (node.isRoot()) {
                            ...
                            mDepartmentData.testbiaoshiid = node.getpId();
                        mDepartmentData.departtype = Constants.DEPARTTYPE_OWNER;
                        mDepartmentData.testdeparttype = Constants.DEPARTTYPE_OWNER;
                        mDepartmentData.departmentName = node.getName();
                    } else {
                        if (node.getLevel() == 1) {
                            mDepartmentData.departtype = Constants.DEPARTTYPE_OWNER;
                            mDepartmentData.testdeparttype = Constants.DEPARTTYPE_SECTION;
                            mDepartmentData.equipmentID = node.getEquipmentId();
                            mDepartmentData.testbiaoshiid = node.getpId().substring(0,2);
                            mDepartmentData.departmentName = node.getName();
                        } else if (node.getLevel() == 2) {
                            mDepartmentData.departtype = Constants.DEPARTTYPE_OWNER;
                            mDepartmentData.testdeparttype = Constants.DEPARTTYPE_PROJECT;
                            mDepartmentData.testbiaoshiid = node.getpId().substring(0,2);
                            mDepartmentData.departmentName = node.getName();
                        } else if (node.getLevel() == 3) {
                            mDepartmentData.departtype = Constants.DEPARTTYPE_OWNER;
                            mDepartmentData.testdeparttype = Constants.DEPARTTYPE_BHZ;
                            mDepartmentData.equipmentID = node.getEquipmentId();
                            mDepartmentData.testbiaoshiid = node.getpId().substring(0,2);
                            mDepartmentData.departmentName = node.getName();
                        }
...
//                EventBus.getDefault().postSticky(new EventData(mDepartmentData));
                ...
                intent.putExtra("departtype",mDepartmentData.testdeparttype);
                intent.putExtra("biaoshiid",mDepartmentData.testbiaoshiid.substring(0,2));
                intent.putExtra("departmentName",mDepartmentData.departmentName);
                setResult(5, intent);
