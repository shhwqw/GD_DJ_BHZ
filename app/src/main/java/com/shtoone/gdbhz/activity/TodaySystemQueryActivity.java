...
public class TodaySystemQueryActivity extends BaseActivity{
    ...
    private List<TodaySystemQueryData.DataEntity> listData;
    private TodaySystemQueryData                  itemsData;
    private TodaySystemQueryAdapter               mAdapter;
    ...
    private void initData() {
        ...
        if (null != BaseApplication.mDepartmentData && !TextUtils.isEmpty(BaseApplication.mDepartmentData.departmentName)) {
            mDepartmentData = new DepartmentData(BaseApplication.mUserInfoData.getUserType(), BaseApplication.mUserInfoData.getUserFullName(), ConstantsUtils.TODAYSYSTEMQUERY);
            mParametersData = (ParametersData) BaseApplication.parametersData.clone();
            mParametersData.departtype = BaseApplication.mUserInfoData.getUserType();
        }
        mParametersData.fromTo = ConstantsUtils.TODAYSYSTEMQUERY;
        mGson = new Gson();
            ...
            public boolean onMenuItemClick(MenuItem item) {
                switch (item.getItemId()) {
                    case R.id.action_hierarchy:
                        Intent intent = new Intent(TodaySystemQueryActivity.this, TodayLabOrganizationActivity.class);
                        Bundle bundle = new Bundle();
                        bundle.putSerializable(ConstantsUtils.DEPARTMENT, mDepartmentData);
                        intent.putExtras(bundle);
                        intent.putExtra("modelType", "");
                        AnimationUtils.startActivity(TodaySystemQueryActivity.this, intent, mToolbar.findViewById(R.id.action_hierarchy), R.color.base_color);
                        break;
                }
                return true;
            }
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent = new Intent(TodaySystemQueryActivity.this, DialogActivity.class);
                Bundle bundle = new Bundle();
        ...
        //设置动画与适配器
        SlideInLeftAnimationAdapter mSlideInLeftAnimationAdapter = new SlideInLeftAnimationAdapter(mAdapter = new TodaySystemQueryAdapter(this, listData));
...
//    }
    ...
    public void updateDepartment(DepartmentData mDepartmentData) {
        if (null != mDepartmentData && null != mParametersData ) {
            if (mDepartmentData.fromto == ConstantsUtils.TODAYSYSTEMQUERY) {
                this.mParametersData.departtype = mDepartmentData.testdeparttype;
                this.mParametersData.biaoshiid = mDepartmentData.testbiaoshiid;
    ...
    public String createRefreshULR() {
        ...
        if (null != mParametersData) {
            ...
            endDateTime = sdf.format(rld1.getTime());
            currentPage = mParametersData.currentPage;
            todaylp = mParametersData.todaylp;
            jiaozhubuwei = mParametersData.jiaozhubuwei;
            todaysjqd = mParametersData.todaysjqd;
        }
        ...
        try {
            return URL.getTODAYSYSTEMQUERY_URL(departType,biaoshiid,startDateTime,endDateTime,currentPage,todaylp,todaysjqd,URLEncoder.encode(jiaozhubuwei,"utf-8"));
    ...
    public void onRefreshSuccess(String response) {
        if (!TextUtils.isEmpty(response)) {
            ...
            if (jsonObject.optBoolean("success")) {
                itemsData = mGson.fromJson(response, TodaySystemQueryData.class);
                if (null != itemsData) {
    ...
    public void updateSearch(ParametersData mParametersData) {
        if (mParametersData != null) {
            if (mParametersData.fromTo == ConstantsUtils.TODAYSYSTEMQUERY) {
