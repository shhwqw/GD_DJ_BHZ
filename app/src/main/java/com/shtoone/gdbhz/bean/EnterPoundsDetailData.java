package com.shtoone.gdbhz.bean;

public class EnterPoundsDetailData {

    /**
     * data : {"maozhong":"50.82","jinchangshijian":"2016-11-26 14:10:38","jingzhong":"36.93","gongyingshangname":"安徽祥实商贸有限公司","remark":"","pici":"","CCHCPPic":"","JCGKPic":"","chengzhongpiancha":"","JCBFPic":"","JCCPPic":"","kouzhong":"0.00","liaocang":"","chuchangshijian":"2016-11-26 14:13:05","qianchepai":"皖A85035","pizhong":"13.14","cailiaoname":"碎石(16-31.5mm)","CCCPPic":"","koulv":"98","CCGKPic":"","sibangyuan":"系统管理员","JCHCPPic":"","CCBFPic":"","banhezhanminchen":"1号拌合站地磅","jinchuliaodanno":"20161126141927"}
     * success : true
     */

    private DataBean data;
    private boolean success;

    public DataBean getData() {
        return data;
    }

    public void setData(DataBean data) {
        this.data = data;
    }

    public boolean isSuccess() {
        return success;
    }

    public void setSuccess(boolean success) {
        this.success = success;
    }

    public static class DataBean {
        /**
         * maozhong : 50.82
         * jinchangshijian : 2016-11-26 14:10:38
         * jingzhong : 36.93
         * gongyingshangname : 安徽祥实商贸有限公司
         * remark :
         * pici :
         * CCHCPPic :
         * JCGKPic :
         * chengzhongpiancha :
         * JCBFPic :
         * JCCPPic :
         * kouzhong : 0.00
         * liaocang :
         * chuchangshijian : 2016-11-26 14:13:05
         * qianchepai : 皖A85035
         * pizhong : 13.14
         * cailiaoname : 碎石(16-31.5mm)
         * CCCPPic :
         * koulv : 98
         * CCGKPic :
         * sibangyuan : 系统管理员
         * JCHCPPic :
         * CCBFPic :
         * banhezhanminchen : 1号拌合站地磅
         * jinchuliaodanno : 20161126141927
         */

        private String maozhong;
        private String jinchangshijian;
        private String jingzhong;
        private String gongyingshangname;
        private String remark;
        private String pici;
        private String CCHCPPic;
        private String JCGKPic;
        private String chengzhongpiancha;
        private String JCBFPic;
        private String JCCPPic;
        private String kouzhong;
        private String liaocang;
        private String chuchangshijian;
        private String qianchepai;
        private String pizhong;
        private String cailiaoname;
        private String CCCPPic;
        private String koulv;
        private String CCGKPic;
        private String sibangyuan;
        private String JCHCPPic;
        private String CCBFPic;
        private String banhezhanminchen;
        private String jinchuliaodanno;

        public String getMaozhong() {
            return maozhong;
        }

        public void setMaozhong(String maozhong) {
            this.maozhong = maozhong;
        }

        public String getJinchangshijian() {
            return jinchangshijian;
        }

        public void setJinchangshijian(String jinchangshijian) {
            this.jinchangshijian = jinchangshijian;
        }

        public String getJingzhong() {
            return jingzhong;
        }

        public void setJingzhong(String jingzhong) {
            this.jingzhong = jingzhong;
        }

        public String getGongyingshangname() {
            return gongyingshangname;
        }

        public void setGongyingshangname(String gongyingshangname) {
            this.gongyingshangname = gongyingshangname;
        }

        public String getRemark() {
            return remark;
        }

        public void setRemark(String remark) {
            this.remark = remark;
        }

        public String getPici() {
            return pici;
        }

        public void setPici(String pici) {
            this.pici = pici;
        }

        public String getCCHCPPic() {
            return CCHCPPic;
        }

        public void setCCHCPPic(String CCHCPPic) {
            this.CCHCPPic = CCHCPPic;
        }

        public String getJCGKPic() {
            return JCGKPic;
        }

        public void setJCGKPic(String JCGKPic) {
            this.JCGKPic = JCGKPic;
        }

        public String getChengzhongpiancha() {
            return chengzhongpiancha;
        }

        public void setChengzhongpiancha(String chengzhongpiancha) {
            this.chengzhongpiancha = chengzhongpiancha;
        }

        public String getJCBFPic() {
            return JCBFPic;
        }

        public void setJCBFPic(String JCBFPic) {
            this.JCBFPic = JCBFPic;
        }

        public String getJCCPPic() {
            return JCCPPic;
        }

        public void setJCCPPic(String JCCPPic) {
            this.JCCPPic = JCCPPic;
        }

        public String getKouzhong() {
            return kouzhong;
        }

        public void setKouzhong(String kouzhong) {
            this.kouzhong = kouzhong;
        }

        public String getLiaocang() {
            return liaocang;
        }

        public void setLiaocang(String liaocang) {
            this.liaocang = liaocang;
        }

        public String getChuchangshijian() {
            return chuchangshijian;
        }

        public void setChuchangshijian(String chuchangshijian) {
            this.chuchangshijian = chuchangshijian;
        }

        public String getQianchepai() {
            return qianchepai;
        }

        public void setQianchepai(String qianchepai) {
            this.qianchepai = qianchepai;
        }

        public String getPizhong() {
            return pizhong;
        }

        public void setPizhong(String pizhong) {
            this.pizhong = pizhong;
        }

        public String getCailiaoname() {
            return cailiaoname;
        }

        public void setCailiaoname(String cailiaoname) {
            this.cailiaoname = cailiaoname;
        }

        public String getCCCPPic() {
            return CCCPPic;
        }

        public void setCCCPPic(String CCCPPic) {
            this.CCCPPic = CCCPPic;
        }

        public String getKoulv() {
            return koulv;
        }

        public void setKoulv(String koulv) {
            this.koulv = koulv;
        }

        public String getCCGKPic() {
            return CCGKPic;
        }

        public void setCCGKPic(String CCGKPic) {
            this.CCGKPic = CCGKPic;
        }

        public String getSibangyuan() {
            return sibangyuan;
        }

        public void setSibangyuan(String sibangyuan) {
            this.sibangyuan = sibangyuan;
        }

        public String getJCHCPPic() {
            return JCHCPPic;
        }

        public void setJCHCPPic(String JCHCPPic) {
            this.JCHCPPic = JCHCPPic;
        }

        public String getCCBFPic() {
            return CCBFPic;
        }

        public void setCCBFPic(String CCBFPic) {
            this.CCBFPic = CCBFPic;
        }

        public String getBanhezhanminchen() {
            return banhezhanminchen;
        }

        public void setBanhezhanminchen(String banhezhanminchen) {
            this.banhezhanminchen = banhezhanminchen;
        }

        public String getJinchuliaodanno() {
            return jinchuliaodanno;
        }

        public void setJinchuliaodanno(String jinchuliaodanno) {
            this.jinchuliaodanno = jinchuliaodanno;
        }
    }
}
